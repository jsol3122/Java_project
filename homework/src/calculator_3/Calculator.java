...
public class Calculator extends JFrame implements ActionListener {
	...
	private double result=0; // 처음값, 중간결과값 저장
	private char op=0; 
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(	e.getActionCommand()=="0" ||
			...
			e.getActionCommand()=="9") {	// 숫자 입력 
			if(dispBuffer.length()==0 && e.getActionCommand()=="0") return; // 처음 0만 누르기 금지
		inputL.setText("");
		...
		inputL.setText(buffer.toString());
			...
			if(buffer.indexOf(".")==-1) {
				...
				inputL.setText(buffer.toString());
				 ...
				 e.getActionCommand()=="*" ||
				 e.getActionCommand()=="/" ||
				 e.getActionCommand()=="=") {	// +-*/= 연산자 처리 --------------------------------------------
			if(op==0 && buffer.length()>0) { // 첫 연산자일 때
			...
			}else if(op!=0 && buffer.length()>0) { // 연산자 이미 있을 때
				if(op=='+') {
				result += Double.parseDouble(buffer.toString());
				buffer.delete(0, buffer.length());
				inputL.setText("");
